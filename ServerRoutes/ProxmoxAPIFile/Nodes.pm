...
__PACKAGE__->register_method({
	...
	#----------------------------------------------------------GPU TEMP--------------------------------------------------------------------------------
	# You must have NVIDIA-SMI installed for GPU to be detetced. ONLY WORKS ON NVIDIA GPUS.
	my $gpu_info_output = `nvidia-smi --query-gpu=utilization.gpu,temperature.gpu,memory.total,memory.used, --format=csv,noheader,nounits`;
        chomp($gpu_info_output);
        my ($gpu_usage, $gpu_temp, $gpu_memory_total, $gpu_memory_used) = split(',', $gpu_info_output);

        $res->{gpu} = {
        	gpu_usage => int($gpu_usage),
        	gpu_temp  => int($gpu_temp),
	    	gpu_memory_used => int($gpu_memory_used),
	    	gpu_memory_total => int($gpu_memory_total)
		};
